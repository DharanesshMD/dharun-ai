...
const initializeWorker = () => {
    ...
    // If SharedWorker isn't available in the environment, immediately fallback
    if (typeof SharedWorker === 'undefined') {
        initializeTabSync();
        return;
    }

    // Try to safely resolve import.meta.url to a worker URL. In some bundlers / dev
    // setups import.meta.url may be unavailable or invalid which causes `new URL(...)`
    // to throw. Do a guarded resolution and fall back to localStorage sync.
    let workerUrl: string | null = null;
    try {
        // Some bundlers / TS configs disallow direct references to `import.meta` at
        // parse-time. Use eval to access it at runtime only. If unavailable, fall
        // back to the localStorage tab-sync implementation.
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const metaUrl = (() => {
            try {
                // Access import.meta dynamically to avoid TS parsing errors
                // and environments that don't expose it.
        ...
        if (metaUrl) {
            workerUrl = new URL('./db-sync.worker.ts', metaUrl).toString();
        }
    } catch (err) {
        // Resolution failed; we'll fallback below
        workerUrl = null;
    }

    if (!workerUrl) {
        // Can't construct a valid worker URL in this environment; use storage-based sync
        initializeTabSync();
        return;
    }

    try {
        // Create a shared worker using the resolved URL
        dbWorker = new SharedWorker(workerUrl, { type: 'module' });
        ...
        // Fallback to localStorage method if SharedWorker isn't supported or creation failed
        initializeTabSync();
    }
};
...
if (typeof window !== 'undefined') {
    ...
    ({ threads, currentThreadId, chatMode, showSuggestions, customInstructions }) => {
            useChatStore.setState({
                threads,
                currentThreadId,
